...
object UserFlow {
  object Registration {
    ...
    val newPatientOpdVisit: ScenarioConfig =
      ScenarioConfig("Registration - New Patient - OPD Visit", 50, 3 minutes)
    val existingPatientIdSearchOpdVisit: ScenarioConfig =
      ScenarioConfig("Registration - Existing Patient ID Search - OPD Visit", 30, 3 minutes)
    val existingPatientNameSearchOpdVisit: ScenarioConfig =
      ScenarioConfig("Registration - Existing Patient Name Search - OPD Visit", 20, 3 minutes)
  }
  object Consultation {
    val trafficLoadShare: Int = 50

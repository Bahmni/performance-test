...
object Common {
  ...
  def closeVisit(): ChainBuilder = {
    exec(getVisitLocation(LOGIN_LOCATION_UUID))
      ...
      .exec(getProviderForUser("#{runTimeUuid}"))
      .exec(
        getRegistrationConcepts.resources(
      ...
      .exec(
        findEncounter("#{opdPatientId}", PROVIDER_UUID, CLOSE_VISIT_ENCOUNTER_TYPE_UUID)
          .resources(
  ...
  def waitBeforeNextWorkLoad(expectedCompletionTime: FiniteDuration): ChainBuilder = {
    exec { session =>
      val timeElapsed = (System.currentTimeMillis() - session("startTime").as[Long]) / 1000
